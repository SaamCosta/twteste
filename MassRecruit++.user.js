...
function generateUnitArrays() {
  var count = units.barrack.length + units.stable.length + units.garage.length; //Amount of units in the game
...
try {
    for (var j = 0; j < trainingLength; j++) { //Strange for loop is to reserve a space for the list item in the first slot
        ...
        for (var i = 0; i < units.barrack.length; i++) {
            ...
            if (hasClassRest) {
                ...
                var parsedUnits = units.barrack[i] + "," + amountTroopsLeft;
                trainingBarrack[j + 1] = parsedUnits;
                logBarrack(1, "Unit " + j + " has type: " + units.barrack[i]);
                logBarrack(1, "And has " + amountTroopsLeft + " units to train");
                logBarrack(1, "----------");
            } else {
                //logBarrack(1, "Nope. Looked for type: " + units.barrack[i]);
            }
        }
    }
} catch (ex) {
